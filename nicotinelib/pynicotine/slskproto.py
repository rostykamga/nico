...
class SlskProtoThread(threading.Thread):
	...
	def __init__(self, ui_callback, queue, bindip, config, eventprocessor):
		...
		if listenport is not None:
			...
			short = ("Could not bind to a local port, aborting connection")
			long = ("The range you specified for client connection ports was %(low)s-%(high)s, but none of these were usable. Increase and/or move the range and restart Nicotine+.") % {'low':portrange[0], 'high':portrange[1]}
			if  portrange[0] < 1024:
				long += "\n\n" + ("Note that part of your range lies below 1024, this is usually not allowed on most operating systems with the exception of Windows.")
	...
	def run(self):
		...
		while not self._want_abort:
			...
			try:
				...
				self._want_abort = 1
				message = ("Major Socket Error: Networking terminated! %s" % str(error))
			...
			if p in input[:]:
				try:
					...
					if self.ipBlocked(ip):
						message = ("Ignoring connection request from blocked IP Address %(ip)s:%(port)s" % {
							'ip': ip,
							'port': port
						})
						log.add(message, 3)
			...
			if msgsize + 4 > len(msgBuffer):
				...
				if msgtype in self.peerclasses:
						...
						try:
							...
							if "addr" in conn.__dict__:
								if conn.addr is not None:
									host = conn.addr[0]
									port = conn.addr[1]
							debugmessage = ("There was an error while unpacking Peer message type %(type)s size %(size)i contents %(msgBuffer)s from user: %(user)s, %(host)s:%(port)s") %{'type':msgname, 'size':msgsize-4, 'msgBuffer':msgBuffer[8:msgsize+4].__repr__(), 'user':conn.init.user, 'host': host, 'port': port}
							print debugmessage
					...
					for char in msgBuffer[8:msgsize+4].__repr__():
						...
						newbuf += char
						x += 1
					debugmessage = ("Peer message type %(type)s size %(size)i contents %(msgBuffer)s unknown, from user: %(user)s, %(host)s:%(port)s") %{'type':msgtype, 'size':msgsize-4, 'msgBuffer': newbuf, 'user':conn.init.user, 'host': host, 'port': port}
					msgs.append(debugmessage)
					print debugmessage
		...
		for msgObj in msgList:
			if issubclass(msgObj.__class__, ServerMessage):
				try:
					...
					self._ui_callback([("Error packaging message: %(type)s %(msg_obj)s, %(error)s") %{'type':msgObj.__class__, 'msg_obj':vars(msgObj), 'error': str(error)}])
				...
				if msgObj.conn in conns:
					...
					if msgObj.__class__ not in [PeerInit, PierceFireWall, FileSearchResult]:
						message = ("Can't send the message over the closed connection: %(type)s %(msg_obj)s") %{'type':msgObj.__class__, 'msg_obj':vars(msgObj)}
						log.add(message, 3)
				...
				if socketwarning and time.time() - self.lastsocketwarning > 60:
					...
					log.addwarning(("You have just hit your connection limit of %(limit)s. Nicotine+ will drop connections for your protection. If you get this message often you should search for less generic terms, or increase your per-process file descriptor limit.") % {'limit':maxsockets})
		if needsleep:
			time.sleep(1)
