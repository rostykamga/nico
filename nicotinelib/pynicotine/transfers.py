...
class Transfers:
	...
	def getAddUser(self, msg):
		...
		if not msg.userexists and self.eventprocessor.config.sections["ui"]["notexists"]:
			self.eventprocessor.logMessage(("User %s does not exist") % (msg.user), 1)
	...
	def UploadFailed(self, msg):
		...
		for i in self.downloads:
			if i.user == user and i.filename == msg.file and (i.conn is not None or i.status in ["Connection closed by peer", "Establishing connection", "Waiting for download"]):
				...
				self.getFile(i.user, i.filename, i.path, i)
				self.eventprocessor.logTransfer(("Retrying failed download: user %(user)s, file %(file)s") %{'user':i.user, 'file':self.decode(i.filename)}, 1)
				break
		else:
			self.eventprocessor.logTransfer(("Failed download: user %(user)s, file %(file)s") %{'user':user, 'file':self.decode(msg.file)}, 1)
	...
	def TransferRequestUploads(self, msg, user, conn, addr):
		...
		self.eventprocessor.logMessage(("Upload request: %(req)s Response: %(resp)s") % {
			'req': str(vars(msg)),
			'resp': response
		}, 5)
		return response
	...
	def QueueUpload(self, msg):
		...
		self.eventprocessor.logMessage(("Queued upload request: User %(user)s, %(msg)s") % {
			'user': user,
			'msg': str(vars(msg))
		}, 5)
		self.checkUploadQueue()
	...
	def UploadQueueNotification(self, msg):
		...
		if self.CanUpload(username):
			self.eventprocessor.logMessage(("Your buddy, %s, is attempting to upload file(s) to you.")%(username), None)
			if username not in self.RequestedUploadQueue:
				self.RequestedUploadQueue.append(username)
		else:
			self.queue.put(slskmessages.MessageUser(username, ("[Automatic Message] ")+("You are not allowed to send me files.")) )
			self.eventprocessor.logMessage(("%s is not allowed to send you file(s), but is attempting to, anyway. Warning Sent.")%(username), None)
			return
	...
	def FileRequest(self, msg):
		...
		for i in self.uploads:
			if msg.req == i.req:
				...
				return
		self.eventprocessor.logMessage(("Unknown file request: %s") % str(vars(msg)), 1)
	...
	def _FileRequestDownload(self, msg, i):
		...
		if i.conn is None and i.size is not None:
			...
			try:
				...
				self.eventprocessor.logMessage(("OS error: %s") % strerror)
				i.status = "Download directory error"
				i.conn = None
				self.queue.put(slskmessages.ConnClose(msg.conn))
				self.eventprocessor.frame.NewNotification(("OS error: %s") % strerror, title=_("Nicotine+ :: Directory download error"))
				...
				try:
					...
					self.eventprocessor.logMessage(("Download I/O error: %s") % strerror)
					...
					if self.eventprocessor.config.sections["transfers"]["lock"]:
						try:
							...
							try:
								...
								self.eventprocessor.logMessage(("Can't get an exclusive lock on file - I/O error: %s") % strerror)
						except ImportError:
							pass
					f.seek(0, 2)
					size = f.tell()
					...
					self.eventprocessor.logMessage(("Download started: %s") % (u"%s" % f.name), 5)

					self.eventprocessor.logTransfer(("Download started: user %(user)s, file %(file)s") % {'user':i.user, 'file':u"%s" % f.name}, 5)
			self.SetIconDownloads()
			self.downloadspanel.update(i)
		else:
			self.eventprocessor.logMessage(("Download error formally known as 'Unknown file request': %(req)s (%(user)s: %(file)s)") % {
				'req': str(vars(msg)),
				'user': i.user,
				'file': i.filename
			}, 1)
	...
	def _FileRequestUpload(self, msg, i):
		if i.conn is None:
			...
			try:
				...
				i.status = "Initializing transfer"
				i.file = f
				self.eventprocessor.logTransfer(("Upload started: user %(user)s, file %(file)s") % {'user':i.user, 'file':self.decode(i.filename)})
			except IOError, strerror:
				self.eventprocessor.logMessage(("Upload I/O error: %s") % strerror)
				i.status = "Local file error"
				try:
					f.close()
				except:
			...
			self.eventprocessor.logMessage(("Upload error formally known as 'Unknown file request': %(req)s (%(user)s: %(file)s)") % {
				'req': str(vars(msg)),
				'user': i.user,
				'file': i.filename
			}, 1)
			...
			try:
				...
				if i.size > i.currentbytes:
					...
					if newname:
						try:
							...
							try:
								...
								log.addwarning(("Couldn't move '%(tempfile)s' to '%(file)s': %(error)s") % {'tempfile':self.decode(msg.file.name), 'file':self.decode(newname), 'error':str(inst)})
					i.status = "Finished"
					if newname:
						self.eventprocessor.logMessage(("Download finished: %(file)s") % {'file':self.decode(newname)}, 5)
						self.eventprocessor.logTransfer(("Download finished: user %(user)s, file %(file)s") % {'user':i.user, 'file':self.decode(i.filename)})
					else:
						self.eventprocessor.logMessage(("File %(file)s is identical to %(identical)s, not saving.") % {'file':self.decode(msg.file.name), 'identical':identicalfile})
						self.eventprocessor.logTransfer(("Download finished but not saved since it's a duplicate: user %(user)s, file %(file)s") % {'user':i.user, 'file':self.decode(i.filename)})
					...
					if config["transfers"]["shownotification"]:
						self.eventprocessor.frame.NewNotification(("%(file)s downloaded from %(user)s") % {'user':i.user, "file":newname.rsplit(os.sep, 1)[1]}, title=_("Nicotine+ :: file downloaded"))

					if newname and config["transfers"]["afterfinish"]:
						if not executeCommand(config["transfers"]["afterfinish"], newname):
							self.eventprocessor.logMessage(("Trouble executing '%s'") % config["transfers"]["afterfinish"])
						else:
							self.eventprocessor.logMessage(("Executed: %s") % config["transfers"]["afterfinish"])
					if i.path and (config["transfers"]["shownotificationperfolder"] or config["transfers"]["afterfolder"]):
						...
						for ia in self.downloads:
							...
							if config["transfers"]["shownotificationperfolder"]:
								self.eventprocessor.frame.NewNotification(("%(folder)s downloaded from %(user)s") % {'user':i.user, "folder":folder}, title=_("Nicotine+ :: directory completed"))
							if config["transfers"]["afterfolder"]:
								if not executeCommand(config["transfers"]["afterfolder"], folder):
									self.eventprocessor.logMessage(("Trouble executing on folder: %s") % config["transfers"]["afterfolder"])
								else:
									self.eventprocessor.logMessage(("Executed on folder: %s") % config["transfers"]["afterfolder"])
			except IOError, strerror:
				self.eventprocessor.logMessage(("Download I/O error: %s") % self.decode(strerror))
				i.status = "Local file error"
				try:
					msg.file.close()
				except:
		...
		for i in self.uploads:
			...
			if i.size > i.currentbytes:
				...
				for j in self.uploads:
					if j.user == i.user:
						j.timequeued = curtime
				self.eventprocessor.logTransfer(("Upload finished: %(user)s, file %(file)s") % {'user':i.user, 'file':self.decode(i.filename)})
				self.checkUploadQueue()
				self.uploadspanel.update(i)
	...
	def AbortTransfer(self, transfer, remove = 0):
		...
		if transfer.file is not None:
			...
			if transfer in self.uploads:
				self.eventprocessor.logTransfer(("Upload aborted, user %(user)s file %(file)s") % {'user':transfer.user, 'file':transfer.filename})
			else:
				self.eventprocessor.logTransfer(("Download aborted, user %(user)s file %(file)s") % {'user':transfer.user, 'file':transfer.filename})
