...
class NetworkEventProcessor:
    ...
    thread"""
    def __init__(self, nicotineapi, callback, writelog, setstatus, bindip, configfile):

        self.nicotineapi = nicotineapi
        self.callback = callback
        ...
        try:
            ...
            try:
                ...
                self.geoip = None
        self.protothread = slskproto.SlskProtoThread(self.nicotineapi.networkcallback, self.queue, self.bindip, self.config, self)
        ...
        from api import chatrooms
        from api import search
        self.chatrooms = chatrooms.ChatRooms(self.nicotineapi, self.config.sections)
        self.privatechat = None
        self.globallist = None
        self.userinfo = None
        self.userbrowse = None
        self.search = search.Searches(self.nicotineapi)
        self.transfers = None
        self.userlist = None
        self.logintime = None
        self.ipaddress = None
    ...
    def ProcessRequestToPeer(self, user, message, window = None, address = None):
        ...
        for i in self.peerconns:
            if i.username == user and i.init.type == 'P' and message.__class__ is not slskmessages.FileRequest:
                peerconn = i
                break
        if peerconn is not None:
            if peerconn.conn is not None:
                message.conn = peerconn.conn
                self.queue.put(message)
                if window is not None:
                    window.InitWindow(peerconn.username, peerconn.conn)
                if message.__class__ is slskmessages.TransferRequest and self.transfers is not None:
                    self.transfers.gotConnect(message.req, peerconn.conn)
                return
            else:
                peerconn.msgs.append(message)
        else:
            ...
            peerconn = PeerConnection(addr = addr, username = user, msgs = [message], token = token, init = init)
            self.peerconns.append(peerconn)
            ...
            if message.__class__ is slskmessages.FileSearchResult:
                self.searchResultsConnections.append(peerconn)
        if message.__class__ is slskmessages.TransferRequest and self.transfers is not None:
            if peerconn.addr is None:
                self.transfers.gettingAddress(message.req)
            elif peerconn.token is None:
    ...
    def setServerTimer(self):
        ...
        logging.info(("The server seems to be down or not responding, retrying in %i seconds") %(self.servertimeout))
    ...
    def ConnectToServer(self, msg):
        self.nicotineapi.OnConnect(None)
    ...
    def PopupMessage(self, msg):
        self.setStatus((msg.title))
        self.nicotineapi.PopupMessage(msg)

    def DebugMessage(self, msg):
        self.logMessage(msg.msg, msg.debugLevel)

    def DisplaySockets(self, msg):
        self.nicotineapi.SetSocketStatus(msg.msg)

    def ConnectError(self, msg):
        if msg.connobj.__class__ is slskmessages.ServerConn:
            self.setStatus(("Can't connect to server %(host)s:%(port)s: %(error)s") % {'host': msg.connobj.addr[0], 'port': msg.connobj.addr[1], 'error': self.decode(msg.err) } )
            if msg.connobj.addr[1] == 2240:
                self.logMessage(("WARNING: You tried to connect to the old server, which might have been shut down. To connect to the new Soulseek server edit your settings and change the server port from 2240 to 2242"))
            self.setServerTimer()
            if self.serverconn is not None:
                self.serverconn = None

            self.nicotineapi.ConnectError(msg)
            ...
            for i in self.peerconns[:]:
                if i.addr == msg.connobj.addr and i.conn is None:
                    if i.token is None:
                        ...
                        self.logMessage(("Can't connect to %s, sending notification via the server") %(i.username), 3)
    ...
    def ServerConn(self, msg):
        self.setStatus(("Connected to server %(host)s:%(port)s, logging in...") % {'host':msg.addr[0], 'port': msg.addr[1]})
        if msg.addr[1] == 2240:
            self.logMessage(("WARNING: You are connected to the old server, which will most likely be shut down in the near future. To connect to the new Soulseek server edit your settings and change the server port from 2240 to 2242"))
        time.sleep(1)
        self.serverconn = msg.conn
        self.servertimeout = -1
        self.users = {}
        login, password= self.config.sections["server"]["login"], self.config.sections["server"]["passw"]
        self.queue.put(slskmessages.Login(login, password,157)) #155, 156, 157, 180
        if self.waitport is not None:
            self.queue.put(slskmessages.SetWaitPort(self.waitport))
    ...
    def ClosedConnection(self, conn, addr):
        if conn == self.serverconn:
            self.setStatus(("Disconnected from server %(host)s:%(port)s") %{'host':addr[0], 'port':addr[1]})
            userchoice = bool(self.nicotineapi.manualdisconnect)
            if not self.nicotineapi.manualdisconnect:
                self.setServerTimer()
            else:
                self.nicotineapi.manualdisconnect = 0
            if self.searchResultsTimer is not None:
                self.searchResultsTimer.cancel()
            if self.respondDistributedTimer is not None:
                self.respondDistributedTimer.cancel()
            ...
            self.privatechat  = self.userinfo = self.userbrowse = self.search = self.transfers = self.userlist = None
            self.nicotineapi.ConnClose(conn, addr)
            #self.nicotineapi.pluginhandler.ServerDisconnectNotification(userchoice)
        else:
            for i in self.peerconns[:]:
                if i.conn == conn:
                    self.logMessage(("Connection closed by peer: %s") % self.decode(vars(i)), 3)
                    if i.conntimer is not None:
                        i.conntimer.cancel()
                    if self.transfers is not None:
                ...
                self.logMessage(("Removed connection closed by peer: %(conn_obj)s %(address)s") %{'conn_obj':conn, 'address':addr}, 3)
    ...
    def Login(self, msg):
        ...
        if msg.success:
            ...
            #self.privatechat, self.chatrooms, self.userinfo, self.userbrowse, self.search, downloads, uploads, self.userlist = self.nicotineapi.InitInterface(msg)

            #self.transfers.setTransferPanels(downloads, uploads)
            self.shares.sendNumSharedFoldersFiles()
            self.queue.put(slskmessages.SetStatus((not self.nicotineapi.away) + 1))
            ...
            self.nicotineapi.manualdisconnect = 1
            self.setStatus(("Can not log in, reason: %s") %(msg.reason))
            self.logMessage(("Can not log in, reason: %s") %(msg.reason))
        #self.nicotineapi.settingswindow.SetSettings(self.config.sections)
        #self.nicotineapi.settingswindow.SwitchToPage("Server")
        self.nicotineapi.serverConnectionEvent.msg = msg
        self.nicotineapi.serverConnectionEvent.set()

    def ChangePassword(self, msg):
        password = msg.password
        self.config.sections["server"]["passw"] = password
        self.config.writeConfig()
        self.nicotineapi.PopupMessage2(("Your password has been changed"), "Password is %s" % password)
    ...
    def PMessageUser(self, msg):
        ...
        if user == None:
            ...
            return
        if user != msg.user:
            text = ("(Warning: %(realuser)s is attempting to spoof %(fakeuser)s) ") % {"realuser": user, "fakeuser": msg.user} + msg.msg
            msg.user = user
        else:
            text = msg.msg
    ...
    def PublicRoomMessage(self, msg):
        ...
        if self.chatrooms is not None:
            ...
            self.nicotineapi.pluginhandler.PublicRoomMessageNotification(msg.room, msg.user, msg.msg)
        else:
            self.logMessage("%s %s" %(msg.__class__, vars(msg)), 4)
    ...
    def SayChatRoom(self, msg):
        ...
        msg.msg = findBestEncoding(msg.msg, encodings)
        if self.chatrooms is not None:
            event = self.nicotineapi.pluginhandler.IncomingPublicChatEvent(msg.room, msg.user, msg.msg)
            if event != None:
                (r, n, msg.msg) = event
                self.chatrooms.roomsctrl.SayChatRoom(msg, msg.msg)
                self.nicotineapi.pluginhandler.IncomingPublicChatNotification(msg.room, msg.user, msg.msg)
        else:
            self.logMessage("%s %s" %(msg.__class__, vars(msg)), 4)
    ...
    def PrivilegedUsers(self, msg):
        if self.transfers is not None:
            ...
            #self.nicotineapi.pluginhandler.ServerConnectNotification()
        else:
            self.logMessage("%s %s" %(msg.__class__, vars(msg)), 4)
    ...
    def CheckPrivileges(self, msg):
        ...
        hours = mins // 60
        days = hours // 24
        wheretogetthem = " " + ("You can acquire privileges by donating at %(url)s") % {'url':'http://www.slsknet.org/userlogin.php?' + urlencode({'username':self.config.sections["server"]["login"]})}
        if msg.seconds == 0:
            self.logMessage(("You have no privileges left. They are not needed for Nicotine+ to function properly.") + wheretogetthem)
        else:
            self.logMessage(("%(days)i days, %(hours)i hours, %(minutes)i minutes, %(seconds)i seconds of download privileges left.") % {'days':days, 'hours':hours % 24, 'minutes':mins % 60, 'seconds':msg.seconds % 60} + wheretogetthem)
    ...
    def GetUserStatus(self, msg):
        ...
        self.nicotineapi.GetUserStatus(msg)
    ...
    def GetUserStats(self, msg):
        ...
        self.nicotineapi.GetUserStats(msg)
        if self.chatrooms is not None:
        ...
        #self.nicotineapi.pluginhandler.UserStatsNotification(msg.user, stats)
        ...
        for i in self.peerconns:
            if i.username == msg.user and i.addr is None:
                if msg.port != 0 or i.tryaddr == 10:
                    if i.tryaddr == 10:
                        self.logMessage(("Server reported port 0 for the 10th time for user %(user)s, giving up") %{'user':msg.user}, 3)
                    elif i.tryaddr is not None:
                        self.logMessage(("Server reported non-zero port for user %(user)s after %(tries)i retries") %{'user':msg.user, 'tries':i.tryaddr}, 3)
                    i.addr = (msg.ip, msg.port)
                    i.tryaddr = None
                    self.queue.put(slskmessages.OutConn(None, i.addr))
                    self.nicotineapi.getUserAddressEvent.set()
                    for j in i.msgs:
                        if j.__class__ is slskmessages.TransferRequest and self.transfers is not None:
                    ...
                    if i.tryaddr is None:
                        i.tryaddr = 1
                        self.logMessage(("Server reported port 0 for user %(user)s, retrying") %{'user':msg.user}, 3)
                    else:
                        i.tryaddr +=1
            ...
            if msg.user in self.ipblock_requested:
                if self.ipblock_requested[msg.user]:
                    self.nicotineapi.OnUnBlockUser(msg.user)
                else:
                    self.nicotineapi.OnBlockUser(msg.user)
                del self.ipblock_requested[msg.user]
                return
            if msg.user in self.ipignore_requested:
                if self.ipignore_requested[msg.user]:
                    self.nicotineapi.OnUnIgnoreUser(msg.user)
                else:
                    self.nicotineapi.OnIgnoreUser(msg.user)
            ...
            #     self.nicotineapi.pluginhandler.UserResolveNotification(msg.user, msg.ip, msg.port)
            #     return
            ...
            if self.geoip:
               ...
               # if cn is not None:
               #    self.nicotineapi.HasUserFlag(msg.user, "flag_" + cn)
            else:
                cc = ""
            if cc:
                cc = " (%s)" % cc
            ...
            try:
                ...
                message = ("IP address of %(user)s is %(ip)s, name %(host)s, port %(port)i%(country)s") %{'user':msg.user, 'ip':msg.ip, 'host':hostname, 'port':msg.port, 'country':cc}
            except:
                message = ("IP address of %(user)s is %(ip)s, port %(port)i%(country)s") %{'user':msg.user, 'ip':msg.ip, 'port':msg.port, 'country':cc}
            self.logMessage(message)
            #self.nicotineapi.pluginhandler.UserResolveNotification(msg.user, msg.ip, msg.port, cc)

    def Relogged(self, msg):
        self.logMessage(("Someone else is logging in with the same nickname, server is going to disconnect us"))
        self.nicotineapi.manualdisconnect = 1
        #self.nicotineapi.pluginhandler.ServerDisconnectNotification(False)
        ...
        if self.users[user].addr != None:
            ...
            if len(self.users[user].addr) == 2:
                if self.users[user].addr is not None:
                    ...
                    if u_ip != ip:
                        warning = ("IP %(ip)s:%(port)s is spoofing user %(user)s with a peer request, blocking because it does not match IP: %(real_ip)s") %{'ip':ip, 'port':port, 'user':user, 'real_ip':u_ip}
    ...
    def UserInfoRequest(self, msg):
        ...
        if user == self.config.sections["server"]["login"]:
            if ip is not None and port is not None:
                self.logMessage(("Blocking %(user)s from making a UserInfo request, possible spoofing attempt from IP %(ip)s port %(port)s") %{'user':user, 'ip':ip, 'port':port}, 1)
            else:
                self.logMessage(("Blocking %s from making a UserInfo request, possible spoofing attempt from an unknown IP & port") %(user), 1)
            if msg.conn.conn != None:
                self.queue.put(slskmessages.ConnClose(msg.conn.conn))
            return
        if user in self.config.sections["server"]["banlist"]:
            self.logMessage(("%(user)s is banned, but is making a UserInfo request") %{'user':user}, 1)
        ...
        if self.transfers is not None:
            ...
            ua = self.nicotineapi.np.config.sections["transfers"]["remotedownloads"]
            if ua:
                uploadallowed = self.nicotineapi.np.config.sections["transfers"]["uploadallowed"]
            else:
                uploadallowed = ua
            self.queue.put(slskmessages.UserInfoReply(msg.conn.conn, descr, pic, totalupl, queuesize, slotsavail, uploadallowed))
        self.logMessage(("%(user)s is making a UserInfo request") %{'user':user}, 1)
        ...
        for i in self.peerconns:
            if i.conn is msg.conn.conn and self.userbrowse is not None:
                if i.username != self.config.sections["server"]["login"]:
                    ...
                    from utils import   Humanize, HumanizeBytes
                    totalsize = 0
                    for dir, files in msg.list:
                        partialsize=0
                        for filedata in files:
                            if filedata[2] < 18446744000000000000:
                                totalsize += filedata[2]
                                partialsize+=filedata[2]
                            else:
                                print "Unbelievable filesize: %s, %s" % (HumanizeBytes(filedata[2]), repr(filedata))
                        result.append({"dir":dir, "size":partialsize, "files":files})
                        return {"username":i.username, "totalsize": HumanizeBytes(self.totalsize), "Dirs":result}
                        #self.nicotineapi.sharedfilelistresultEvent.set()
    ...
    def FileSearchResult(self, msg):
        ...
        for i in self.peerconns:
            if i.conn is msg.conn.conn and self.search is not None:
                ...
                if country is None:
                    country = ""
                results= self.search.ShowResult(msg, i.username, country)
                #print "instant particulier ..."
                #print results
                if results:
                    for r in results:
                        user, filename, size, speed, queue, immediatedl, h_bitrate, length, directory, bitrate, fullpath, country, status = r
                        res.append({"token":msg.token, "user":user, "filename":filename, "size":size, "speed":speed, "queue":queue, "immediatedl":immediatedl,
                                "h_bitrate":h_bitrate, "length":length, "directory":directory, "bitrate":bitrate, "fullpath":fullpath,
                                "country":country, "status":status})

                    #self.nicotineapi.searchResultEvent.results= res
                    #self.nicotineapi.searchResultEvent.set()
        ...
        for i in self.peerconns[:]:
            if i.token == msg.token:
                ...
                if i == self.GetDistribConn():
                    self.DistribConnClosed(i)
                self.peerconns.remove(i)
                self.logMessage(("Can't connect to %s (either way), giving up") % (i.username), 3)
                for j in i.msgs:
                    if j.__class__ in [slskmessages.TransferRequest, slskmessages.FileRequest] and self.transfers is not None:
                        self.transfers.gotCantConnect(j.req)
        ...
        for i in self.peerconns[:]:
            if i == msg.conn:
                if i == self.GetDistribConn():
                    self.DistribConnClosed(i)
                self.peerconns.remove(i)
                self.logMessage(("User %s does not respond to connect request, giving up") % (i.username), 3)
                for j in i.msgs:
                    if j.__class__ in [slskmessages.TransferRequest, slskmessages.FileRequest] and self.transfers is not None:
                        self.transfers.gotCantConnect(j.req)
                ...
                for j in msg.list[i].keys():
                    if os.path.commonprefix([i, j]) == j:
                        ...
                        if numfiles > 100:
                            ...
                            folder = j
            if many:
                self.nicotineapi.download_large_folder(username, folder, files, numfiles, msg)
            else:
    ...
    def TunneledMessage(self, msg):
        if msg.code in self.protothread.peerclasses:
            ...
            self.logMessage(("Unknown tunneled message: %s") %(vars(msg)), 4)
    ...
    def SearchRequest(self, msg):
        ...
        #self.nicotineapi.pluginhandler.SearchRequestNotification(msg.searchterm, msg.user, msg.searchid)
    ...
    def DistribSearch(self, msg):
        ...
        #self.nicotineapi.pluginhandler.DistribSearchNotification(msg.searchterm, msg.user, msg.searchid)
    ...
    def GlobalRecommendations(self, msg):
        self.nicotineapi.GlobalRecommendations(msg)

    def Recommendations(self, msg):
        self.nicotineapi.Recommendations(msg)

    def ItemRecommendations(self, msg):
        self.nicotineapi.ItemRecommendations(msg)

    def SimilarUsers(self, msg):
        self.nicotineapi.SimilarUsers(msg)

    def ItemSimilarUsers(self, msg):
        self.nicotineapi.ItemSimilarUsers(msg)
    ...
    def logTransfer(self, message, toUI = 0):
        if self.config.sections["logging"]["transfers"]:
            ...
            try:
                ...
                self.logMessage(("Couldn't write to transfer log: %s") % error)
        if toUI:
            self.logMessage(message)
