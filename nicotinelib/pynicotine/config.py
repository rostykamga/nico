                ...
                for j in self.sections[i].keys():
                    ...
                    if self.sections[i][j] is None or self.sections[i][j] == '' and i not in ("userinfo", "ui", "ticker", "players", "language") and j not in ("incompletedir", "autoreply", 'afterfinish', 'afterfolder', 'geoblockcc', 'downloadregexp', "language"):
                        ...
                        if self.sections[i][j] is None and self.defaults[i][j] is not None:
                            ...
                            self.nicotineapi.logMessage(("Config option reset to default: Section: %(section)s, Option: %(option)s, to: %(default)s") % {'section':i, 'option':j, 'default':self.sections[i][j]})
                            if errorlevel == 0:
                                errorlevel = 1
                        else:
                            if errorlevel < 2:
                                self.nicotineapi.logMessage(("You need to configure your settings (Server, Username, Password, Download Directory) before connecting..."))
                                errorlevel = 2

                            self.nicotineapi.logMessage(("Config option unset: Section: %(section)s, Option: %(option)s") % {'section':i, 'option':j})
                            self.nicotineapi.settingswindow.InvalidSettings(i, j)
        except Exception, error:
            message = ("Config error: %s") % error
            self.nicotineapi.logMessage(message)
            if errorlevel < 3:
                errorlevel = 3

        if errorlevel > 1:
            self.nicotineapi.settingswindow.SetSettings(self.sections)
    ...
    def readConfig(self):
        ...
        if exists(self.filename+'.transfers.pickle'):
            ...
            try:
                ...
                log.addwarning(("Something went wrong while opening your transfer list: %(error)s") % {'error': str(inst)})
            else:
                try:
                    self.sections['transfers']['downloads'] = cPickle.load(handle)
                except (IOError, EOFError, ValueError), inst:
                    log.addwarning(("Something went wrong while reading your transfer list: %(error)s") % {'error': str(inst)})
            try:
                handle.close()
            except:
                pass
        ...
        for i in self.parser.sections():
            for j in self.parser.options(i):
                ...
                if i not in self.sections:
                    ...
                    log.addwarning(("Unknown config option '%(option)s' in section '%(section)s'") % {'option': j, 'section': i})
        ...
        if _errors:
            log.addwarning(("Failed to process the following databases: %(names)s") % {'names': '\n'.join(_errors)})
            ...
            log.addwarning(("Shared files database seems to be corrupted, rescan your shares"))
    ...
    def writeDownloadQueue(self):
        ...
        try:
            ...
            try:
                ...
                log.addwarning(("Something went wrong while writing your transfer list: %(error)s") % {'error': str(inst)})
        finally:
            try:
                handle.close()
            except:
