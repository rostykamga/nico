...
class UPnPPortMapping:
    ...
    def AddPortMapping(self, frame, np):
        ...
        log.add(('Creating Port Mapping rule via UPnP...'))
        ...
        try:
            ...
            log.addwarning(('UPnP exception: %(error)s') % {'error': str(e)})
            log.addwarning(
                ('Failed to automate the creation of ' +
                    'UPnP Port Mapping rule.'))
            return

        log.add(
            ('Managed to map external WAN port %(externalwanport)s ' +
