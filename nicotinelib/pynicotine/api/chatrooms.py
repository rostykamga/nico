...
class RoomsControl:
    ...
    def __init__(self, nicotineapi, config):
        self.nicotineapi = nicotineapi
        self.joinedrooms = {}
        self.autojoin = 1
        self.rooms = {}
        #config = self.nicotineapi.np.config.sections
    ...
    # 							  [("Room"), 150, "text", self.RoomStatus],
    # 							  [("Users"), -1, "number", self.RoomStatus],
    # 							  )
    ...
    def IsPrivateRoomOwned(self, room):
        if room in self.PrivateRooms:
            if self.PrivateRooms[room]["owner"] == self.nicotineapi.np.config.sections["server"]["login"]:
                return True
        return False
    ...
    def IsPrivateRoomOperator(self, room):
        if room in self.PrivateRooms:
            if self.nicotineapi.np.config.sections["server"]["login"] in self.PrivateRooms[room]["operators"]:
                return True
        return False
    ...
    # 		cellrenderer.set_property("underline", pango.UNDERLINE_NONE)
    # 	self.nicotineapi.CellDataFunc(column, cellrenderer, model, iter)
    #
    ...
    # 		if name not in self.nicotineapi.np.config.sections["server"]["autojoin"]:
    # 			continue
    # 		room_tab_order [ notebook.page_num(room.Main) ] = name
    # 	pos = 1000
    # 	# Add closed autojoined rooms as well
    # 	for name in self.nicotineapi.np.config.sections["server"]["autojoin"]:
    ...
    # 	# Save
    # 	self.nicotineapi.np.config.sections["server"]["autojoin"] = new_autojoin

    # def OnSwitchPage(self, notebook, page, page_num, force=0):
    # 	if self.nicotineapi.MainNotebook.get_current_page() != self.nicotineapi.MainNotebook.page_num(self.nicotineapi.hpaned1) and not force:
    # 		return
    ...
    # 			# Remove hilite
    # 			self.nicotineapi.Notifications.Clear("rooms", None, name)
    #
    # def ClearNotifications(self):
    # 	if self.nicotineapi.MainNotebook.get_current_page() != self.nicotineapi.MainNotebook.page_num(self.nicotineapi.hpaned1):
    # 		return
    # 	page = self.nicotineapi.ChatNotebook.get_nth_page(self.nicotineapi.ChatNotebook.get_current_page())
    # 	for name, room in self.joinedrooms.items():
    # 		if room.Main == page:
    # 			# Remove hilite
    # 			self.nicotineapi.Notifications.Clear("rooms", None, name)
    ...
    # 		if room.Main == page:
    # 			self.nicotineapi.Notifications.Clear("rooms", name)

    # def OnShowRoomList(self, widget):
    #
    # 	self.nicotineapi.show_room_list1.set_active(1)
    #
    # def OnListClicked(self, widget, event):
    # 	if event.button == 1 and event.type == gtk.gdk._2BUTTON_PRESS:
    # 		d = self.nicotineapi.roomlist.RoomsList.get_path_at_pos(int(event.x), int(event.y))
    # 		if d:
    # 			path, column, x, y = d
    # 			room = self.roomsmodel.get_value(self.roomsmodel.get_iter(path), 0)
    # 			if not room in self.joinedrooms:
    # 				self.nicotineapi.np.queue.put(slskmessages.JoinRoom(room))
    # 		return True
    # 	elif event.button == 3:
    ...
    def OnPopupJoin(self, widget):
        self.nicotineapi.np.queue.put(slskmessages.JoinRoom(self.popup_room))

    def OnEnablePrivateRooms(self, widget):
        self.nicotineapi.np.queue.put(slskmessages.PrivateRoomToggle(True))
    ...
    def OnJoinPublicRoom(self, widget):
        ...
        # angle = 0
        # try:
        # 	angle = int(self.nicotineapi.np.config.sections["ui"]["labelrooms"])
        # except Exception, e:
        # 	print e
        # 	pass
        # self.nicotineapi.ChatNotebook.append_page(room.Main, 'Public ', room.OnLeave, angle)
        room.CountUsers()
        self.nicotineapi.np.queue.put(slskmessages.JoinPublicRoom())

    def OnDisablePrivateRooms(self, widget):
        self.nicotineapi.np.queue.put(slskmessages.PrivateRoomToggle(False))

    def OnPopupCreatePrivateRoom(self, room):
        # room = input_box(self.nicotineapi, title=('Nicotine+:') + " " + ("Create Private Room"),
        # 				 message=('Enter the name of the private room you wish to create'),
        # 				 default_text='')
        if room:
            self.nicotineapi.np.queue.put(slskmessages.JoinRoom(room, 1))

    def OnPopupPrivateRoomDisown(self, widget):
        if self.IsPrivateRoomOwned(self.popup_room):
            self.nicotineapi.np.queue.put(slskmessages.PrivateRoomDisown(self.popup_room))
    ...
    def OnPopupPrivateRoomDismember(self, widget):
        if self.IsPrivateRoomMember(self.popup_room):
            self.nicotineapi.np.queue.put(slskmessages.PrivateRoomDismember(self.popup_room))
            del self.PrivateRooms[self.popup_room]
            self.SetPrivateRooms()

    def OnPopupLeave(self, widget):
        self.nicotineapi.np.queue.put(slskmessages.LeaveRoom(self.popup_room))

    def OnPopupRefresh(self, widget):
        self.nicotineapi.np.queue.put(slskmessages.RoomList())
    ...
    def JoinRoom(self, msg):
        ...
        # 	print e
        # 	pass
        # self.nicotineapi.ChatNotebook.append_page(tab.Main, msg.room, tab.OnLeave, angle)

        #self.nicotineapi.searchroomslist[msg.room] = self.nicotineapi.RoomSearchCombo_List.append([msg.room])
        tab.CountUsers()
    ...
    def SetRoomList(self, msg):
        ...
        if self.autojoin:
            ...
            if self.joinedrooms:
                ...
                list = self.nicotineapi.np.config.sections["server"]["autojoin"]
        ...
        self.nicotineapi.getRoomlistEvent.set()
        # self.roomsmodel.append([room, users, 0])
        # self.rooms.append(room)


    # self.nicotineapi.roomlist.RoomsList.set_model(self.roomsmodel)
    # self.roomsmodel.set_sort_func(1, self.PrivateRoomsSort, 1)
    # self.roomsmodel.set_sort_column_id(1, gtk.SORT_DESCENDING)
    # self.roomsmodel.set_default_sort_func(None)
    #self.cols[1].set_sort_indicator(True)

    # if self.nicotineapi.np.config.sections["words"]["roomnames"]:
    # 	self.nicotineapi.chatrooms.roomsctrl.UpdateCompletions()
    # 	self.nicotineapi.privatechats.UpdateCompletions()

    def SetPrivateRooms(self, ownedrooms=[], otherrooms=[]):
        myusername = self.nicotineapi.np.config.sections["server"]["login"]
        for room in ownedrooms:
            try:
                self.PrivateRooms[room[0]]['joined'] = room[1]
                if self.PrivateRooms[room[0]]['owner'] != myusername:
                    log.addwarning(("I remember the room %(room)s being owned by %(previous)s, but the server says its owned by %(new)s.") % {
                        'room': room[0],
    ...
    def PrivateRoomOperatorAdded(self, msg):
        rooms = self.PrivateRooms
        if msg.room in rooms:
            if self.nicotineapi.np.config.sections["server"]["login"] not in rooms[msg.room]["operators"]:
                rooms[msg.room]["operators"].append(self.nicotineapi.np.config.sections["server"]["login"])
    ...
    def PrivateRoomOperatorRemoved(self, msg):
        rooms = self.PrivateRooms
        if msg.room in rooms:
            if self.nicotineapi.np.config.sections["server"]["login"] in rooms[msg.room]["operators"]:
                rooms[msg.room]["operators"].remove(self.nicotineapi.np.config.sections["server"]["login"])
    ...
    def PrivateRoomAdded(self, msg):
        ...
        if room not in rooms:
            ...
            self.nicotineapi.logMessage(_("You have been added to a private room: %(room)s") % {"room":room})
        self.SetPrivateRooms()
    ...
    def PrivateRoomDisown(self, msg):
        if msg.room in self.PrivateRooms:
            if self.PrivateRooms[msg.room]["owner"] == self.nicotineapi.np.config.sections["server"]["login"]:
    ...
    def GetUserAddress(self, user):
        if user not in self.nicotineapi.np.users:
            self.nicotineapi.np.queue.put(slskmessages.GetPeerAddress(user))
        elif self.nicotineapi.np.users[user].addr is None:
            self.nicotineapi.np.queue.put(slskmessages.GetPeerAddress(user))

    def UserJoinedRoom(self, msg):
        print "joinned rooms"
        print self.joinedrooms
        if msg.room in self.joinedrooms:
    ...
    def SayChatRoom(self, msg, text):
        if msg.user in self.nicotineapi.np.config.sections["server"]["ignorelist"]:
            return
        ...
        if msg.user in self.nicotineapi.np.users and type(self.nicotineapi.np.users[msg.user].addr) is tuple:
            ip, port = self.nicotineapi.np.users[msg.user].addr
            if self.nicotineapi.np.ipIgnored(ip):
                return
    ...
    # 	self.nicotineapi.SetTextBG(self.nicotineapi.roomlist.RoomsList)
    # 	self.nicotineapi.SetTextBG(self.nicotineapi.roomlist.CreateRoomEntry)
    # 	self.nicotineapi.SetTextBG(self.nicotineapi.roomlist.SearchRooms)
    #
    # 	for room in self.joinedrooms.values():
    # 		room.ChangeColours()
    #
    # def saveColumns(self):
    # 	for room in self.nicotineapi.np.config.sections["columns"]["chatrooms"].keys()[:]:
    # 		if room not in self.joinedrooms:
    # 			del self.nicotineapi.np.config.sections["columns"]["chatrooms"][room]
    # 	for room in self.joinedrooms.values():
    # 		room.saveColumns()
    ...
    def LeaveRoom(self, msg):
        ...
        if room.logfile is not None:
            room.logfile.close()
            room.logfile = None
        if self.nicotineapi.ChatNotebook.is_tab_detached(room.Main):
            self.nicotineapi.ChatNotebook.attach_tab(room.Main)
        self.nicotineapi.ChatNotebook.remove_page(room.Main)
        room.destroy()

        del self.joinedrooms[msg.room]
        if msg.room[-1:] != ' ': # meta rooms
            self.nicotineapi.RoomSearchCombo_List.remove(self.nicotineapi.searchroomslist[msg.room])
        if self.joinedrooms == {} and not self.nicotineapi.show_room_list1.get_active():
            win = OptionDialog(self.nicotineapi, _("You aren't in any chat rooms.") + " " + _("Open Room List?"), modal=True, status=None, option=False, third="")
            win.connect("response", self.nicotineapi.onOpenRoomList)
            win.set_title(_("Nicotine+")+": "+_("Open Room List?"))
            win.set_icon(self.nicotineapi.images["n"])
            win.show()
    ...
    def UpdateCompletions(self):
        self.clist = []
        config = self.nicotineapi.np.config.sections["words"]
        if config["tab"]:
            config = self.nicotineapi.np.config.sections["words"]
            clist = [self.nicotineapi.np.config.sections["server"]["login"], "nicotine"]

            if config["roomnames"]:
                clist += self.rooms
            if config["buddies"]:
                clist += [i[0] for i in self.nicotineapi.userlist.userlist]
            if config["aliases"]:
                clist += ["/" + k for k in self.nicotineapi.np.config.aliases.keys()]
            if config["commands"]:
                clist += self.CMDS
            self.clist = clist
...
class ChatRoom:
    def __init__(self, roomsctrl, room, users, meta = False):
        ...
        self.frame = roomsctrl.nicotineapi
        ...
        # 									 [("Status"), statusiconwidth, "pixbuf"],
        # 									 [("Country"), 25, "pixbuf"],
        # 									 [("User"), 100, "text", self.UserColumnDraw],
        # 									 [("Speed"), 0, "number", self.frame.CellDataFunc],
        # 									 [("Files"), 0, "number", self.frame.CellDataFunc],
        # 									 )
        ...
        # 	("", None),
        # 	("#" + ("Send _message"), popup.OnSendMessage, gtk.STOCK_EDIT),
        # 	("", None),
        # 	("#" + ("Show IP a_ddress"), popup.OnShowIPaddress, gtk.STOCK_NETWORK),
        # 	("#" + ("Get user i_nfo"), popup.OnGetUserInfo, gtk.STOCK_DIALOG_INFO),
        # 	("#" + ("Brow_se files"), popup.OnBrowseUser, gtk.STOCK_HARDDISK),
        # 	("#" + ("Gi_ve privileges"), popup.OnGivePrivileges, gtk.STOCK_JUMP_TO),
        # 	("", None),
        # 	("$" + ("_Add user to list"), popup.OnAddToList),
        # 	("$" + ("_Ban this user"), popup.OnBanUser),
        # 	("$" + ("_Ignore this user"), popup.OnIgnoreUser),
        # 	("$" + ("B_lock this user's IP Address"), popup.OnBlockUser),
        # 	("$" + ("Ignore this user's IP Address"), popup.OnIgnoreIP),
        # 	("", None),
        # 	("#" + ("Sear_ch this user's files"), popup.OnSearchUser, gtk.STOCK_FIND),
        # 	(1, ("Private rooms"), self.popup_menu_privaterooms, self.OnPrivateRooms),
        # )
        # items = self.popup_menu.get_children()
    ...
    def UserJoinedRoom(self, username, userdata):
        ...
        #     self.frame.logMessage( ("%s joined the room") % username)
    ...
    def CountUsers(self):
        ...
        if numusers > 1:
            print ("%i people in room") % numusers
            # self.LabelPeople.show()
            # self.LabelPeople.set_text(("%i people in room") % numusers)
        if numusers == 1:
            print "You are alone"
            # self.LabelPeople.show()
            # self.LabelPeople.set_text(("You are alone"))
        # else:
        #     self.LabelPeople.hide()
        if not self.room in self.roomsctrl.rooms:
            self.roomsctrl.rooms[self.room]=self.users
    ...
    def GetUserStats(self, user, avgspeed, files):
        if user not in self.users:
            return
        self.users[user]={"avgspeed":avgspeed, "nbfiles":files}
        #self.usersmodel.set(self.users[user], 3, HumanSpeed(avgspeed), 4, Humanize(files), 6, avgspeed, 7, files)
...
class ChatRooms(IconNotebook):
    def __init__(self, nicotineapi, config):
        ...
        self.roomsctrl = RoomsControl(nicotineapi, config)
