...
class WishList(gtk.Dialog):
    ...
    def __init__(self, frame):
        ...
        self.set_title(("Nicotine+ Wishlist"))
        ...
        column = gtk.TreeViewColumn(("Wishes"), gtk.CellRendererText(), text = 0)
...
class Searches:
    ...
    def __init__(self, nicotineapi):
        ...
        self.timer = None
        self.disconnected = 0
        #ui = self.nicotineapi.np.config.sections["ui"]
        #IconNotebook.__init__(self, nicotineapi.images, ui["labelprivate"], ui["tabclosers"], ui["tab_icons"], ui["tab_reorderable"])
        #self.popup_enable()
        #frame.SearchEntryCombo.disable_activate()

        #self.WishListDialog = WishList(nicotineapi)
    ...
    def LoadConfig(self):
        ...
        during import with gtk.glade.XML in nicotineapi.py)
        """
        items = self.nicotineapi.np.config.sections["searches"]["history"]
        templist = []
        for i in items:
        ...
        return templist
    # for i in templist:
    # 	self.nicotineapi.SearchEntryCombo.append_text(i)
    # self.nicotineapi.WishList.connect("clicked", self.WishListDialog.Toggle)

    def SetInterval(self, msg):
    	self.interval = msg.seconds
    	if not self.disconnected:
    		# Create wishlist searches (without tabs)
    		for term in self.nicotineapi.np.config.sections["server"]["autosearch"]:
    ...
    def OnAutoSearch(self, *args):
        ...
        searches = self.nicotineapi.np.config.sections["server"]["autosearch"]
        if not searches:
            return True
    ...
    def OnClearSearchHistory(self):
        ...
        self.nicotineapi.np.config.sections["searches"]["history"] = []
        self.nicotineapi.np.config.writeConfiguration()


    # def OnSearch(self):
    # 	text = self.nicotineapi.SearchEntry.get_text().strip()
    #
    # 	if not text:
    # 		return
    # 	users = []
    # 	room = None
    # 	if self.nicotineapi.SearchMethod.get_active_text() == _("Global"):
    # 		mode = 0
    # 	elif self.nicotineapi.SearchMethod.get_active_text() == _("Rooms"):
    # 		mode = 1
    # 		name = self.nicotineapi.RoomSearchCombo.child.get_text()
    ...
    # 			room = name
    # 	elif self.nicotineapi.SearchMethod.get_active_text() == _("Buddies"):
    # 		mode = 2
    # 	elif self.nicotineapi.SearchMethod.get_active_text() == _("User"):
    # 		mode = 3
    # 		user = self.nicotineapi.UserSearchCombo.child.get_text().strip()
    ...
    # 	feedback = None
    # 	if mode == 0:
    # 		feedback = self.nicotineapi.pluginhandler.OutgoingGlobalSearchEvent(text)
    # 		if feedback != None:
    # 			text = feedback[0]
    # 	elif mode == 1:
    # 		feedback = self.nicotineapi.pluginhandler.OutgoingRoomSearchEvent(room, text)
    # 		if feedback != None:
    # 			(room, text) = feedback
    # 	elif mode == 2:
    # 		feedback = self.nicotineapi.pluginhandler.OutgoingBuddySearchEvent(text)
    # 		if feedback != None:
    # 			text = feedback[0]
    # 	elif mode == 3:
    # 		feedback = self.nicotineapi.pluginhandler.OutgoingUserSearchEvent(users)
    # 		if feedback != None:
    # 			users = feedback[0]
    # 	else:
    ...
    # 		self.nicotineapi.SearchEntry.set_text("")

    def NewWish(self, wish):
        if wish in self.nicotineapi.np.config.sections["server"]["autosearch"]:
            return
        self.nicotineapi.np.config.sections["server"]["autosearch"].append(wish)
    ...
    def DoSearch(self, text, mode, users = [], room = None):
        ...
        #self.nicotineapi.SearchEntryCombo.get_model().clear()
        # templist = []
        # for i in items:
        # 	if i not in templist:
        # 		templist.append(i)
        ...
        self.searches[self.searchid]= [self.searchid, text, tab, mode, False]
        self.searchid += 1
        return self.searchid-1

    def DoGlobalSearch(self, id, text):
        self.nicotineapi.np.queue.put(slskmessages.FileSearch(id, text))

    def DoWishListSearch(self, id, text):
        self.nicotineapi.np.queue.put(slskmessages.WishlistSearch(id, text))

    def DoRoomsSearch(self, id, text, room = None):
        if room != None:
            self.nicotineapi.np.queue.put(slskmessages.RoomSearch(room, id, text))
        else:
            for room in self.nicotineapi.chatrooms.roomsctrl.joinedrooms.keys():
                self.nicotineapi.np.queue.put(slskmessages.RoomSearch(room, id, text))

    def DoBuddiesSearch(self, id, text):
        for users in self.nicotineapi.userlist.userlist:
            self.nicotineapi.np.queue.put(slskmessages.UserSearch(users[0], id, text))

    def DoPeerSearch(self, id, text, users):
        for user in users:
            self.nicotineapi.np.ProcessRequestToPeer(user, slskmessages.FileSearchRequest(None, id, text))
    ...
    def ShowResult(self, msg, username, country):
        ...
        #msg.token
        #print "Message token %d" % msg.token
        #print "Searches %s" % self.searches
        if msg.token not in self.searches:
            return None

        search = self.searches[msg.token]
        if search[2] is not None:
            return search[2].AddResult(msg, username, country)
    ...
    def RemoveAutoSearch(self, id):
        ...
        if search[1] in self.nicotineapi.np.config.sections["server"]["autosearch"]:
            self.nicotineapi.np.config.sections["server"]["autosearch"].remove(search[1])
            self.nicotineapi.np.config.writeConfiguration()
        search[4] = 0
    ...
    def AutoSearch(self, id):
        if id not in self.searches:
            return
        i = self.searches[id]
        if i[1] in self.nicotineapi.np.config.sections["server"]["autosearch"]:
            return
        self.nicotineapi.np.config.sections["server"]["autosearch"].append(i[1])
        self.nicotineapi.np.config.writeConfiguration()
        i[4] = 1
    ...
    def UpdateColours(self):
        for id in self.searches.values():
            if id[2] is None:
                continue
            id[2].ChangeColours()
        self.nicotineapi.SetTextBG(self.WishListDialog.WishlistView)
    ...
    def NonExistantUser(self, user):
        ...
        # 	popup = PopupMenu(self.nicotineapi)
        # 	popup.setup(
...
class Search:
    ...
    def __init__(self, Searches, text, id, mode, remember):
        ...
        self.frame = Searches.nicotineapi
