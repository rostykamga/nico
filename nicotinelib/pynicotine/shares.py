...
class Shares:
    ...
    def _virtualmapping(self):
        ...
        if self.config.sections["transfers"]["sharedownloaddir"]:
            mapping += [(("Downloaded"), self.config.sections["transfers"]["downloaddir"])]
    ...
    def CompressShares(self, sharestype):
        ...
        if streams is None:
            message = ("ERROR: No %(type)s shares database available") % {"type": sharestype}
            print message
    ...
    def GetSharedFileList(self, msg):
        self.logMessage("GetSharedFileList message received %s %s" % (msg.__class__, vars(msg)), 4)
        user = ip = port = None
        ...
        if user == self.config.sections["server"]["login"]:
            if ip != None and port != None:
                self.logMessage(
                    ("%(user)s is making a BrowseShares request, blocking possible spoofing attempt from IP %(ip)s port %(port)s") % {
                        'user': user,
                        'ip': ip,
                        'port': port
                    }, 1)
                ...
                self.logMessage(
                    ("%(user)s is making a BrowseShares request, blocking possible spoofing attempt from an unknown IP & port") % {
                        'user': user
                    }, None)
        ...
        self.logMessage(("%(user)s is making a BrowseShares request") % {
                'user': user
            }, 1)
    ...
    def rescandirs(self, shared, oldmtimes, oldfiles, sharedfilesstreams, yieldfunction, progress=None, name="", rebuild=False):
        ...
        gobject.idle_add(progress.set_text, ("Checking for changes"))
        gobject.idle_add(progress.show)
        gobject.idle_add(progress.set_fraction, 0)

        self.logMessage(("%(num)s directories found before rescan/rebuild") % {"num": len(oldmtimes)})
        ...
        self.logMessage(("%(num)s directories found after rescan/rebuild") % {"num": len(newmtimes)})

        gobject.idle_add(progress.set_text, ("Scanning %s") % name)
        ...
        gobject.idle_add(progress.set_text, ("Building DataBase"))
        if win32:
        ...
        gobject.idle_add(progress.set_text, ("Building Index"))
    ...
    def getDirsMtimesUnicode(self, dirs, yieldcall=None):
        ...
        for directory in dirs:
            ...
            try:
                ...
                message = ("Scanning Directory Error: %(error)s Path: %(path)s") % {'error': errtuple, 'path': u_directory}
                print str(message)
                self.logMessage(message)
    ...
    def getDirsMtimes(self, dirs, yieldcall=None):
        ...
        for directory in dirs:
            ...
            try:
                ...
                message = ("Scanning Directory Error: %(error)s Path: %(path)s") % {'error': errtuple, 'path': directory}
                print str(message)
                self.logMessage(message)
                displayTraceback(sys.exc_info()[2])
                continue
            ...
            for filename in contents:
                ...
                try:
                    ...
                    message = ("Scanning Error: %(error)s Path: %(path)s") % {'error': errtuple, 'path': path}
                    print str(message)
                    self.logMessage(message)
                    continue
                ...
                try:
                    ...
                    try:
                        ...
                        print errtuple2
                    if islink:
                        message = ("Scanning Error: Broken link to directory: \"%(link)s\" from Path: \"%(path)s\". Repair or remove this link.") % {'link': os.readlink(path), 'path': path}
                    else:
                        message = ("Scanning Error: %(error)s Path: %(path)s") % {'error': errtuple, 'path': path}
                    print str(message)
                    self.logMessage(message)
    ...
    def getFileInfoUnicode(self, name, pathname):
        ...
        try:
            ...
            message = ("Scanning File Error: %(error)s Path: %(path)s") % {'error': errtuple, 'path': pathname}
            self.logMessage(message)
            displayTraceback(sys.exc_info()[2])
    ...
    def getByteStream(self, fileinfo):
        ...
        if fileinfo[2] is not None:
            try:
                ...
                log.addwarning(("Found meta data that couldn't be encoded, possible corrupt file: '%(file)s' has a bitrate of %(bitrate)s kbs, a length of %(length)s seconds and a VBR of %(vbr)s" % {
                        'file':    fileinfo[0],
