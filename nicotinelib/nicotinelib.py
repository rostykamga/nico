...
class NicotineConnection:
    ...
    def __init__(self, config, bindip=None):
        self.loginMethodEvent = threading.Event()
        self.joinRoomMethodEvent = threading.Event()
        self.serverConnectionEvent= threading.Event()
        self.bindip= bindip
        self.isinit=True
        self.np = NetworkEventProcessor(self, self.logMessage, self.bindip, config)
            ...
            for i in msgs:
                if i.__class__ in self.np.events:
                    if i.__class__ is slskmessages.ServerConn:
                        self.serverConnectionEvent.msg=i
    ...
    def login(self, login, password, timeout):
        ...
        return self.loginMethodEvent.msg
    ...
    def joinRoom(self, room, timeout=None):
        ...
        return self.joinRoomMethodEvent.msg if self.joinRoomMethodEvent.wait(timeout) else None
    ...
    def connectToServer(self, servername, serverport, timeout):
        ...
        return self.serverConnectionEvent.msg
